...
using RequestHeader = arangodb::fuerte::RequestHeader;
...
#warning TODO: add database flag
  ...
  auto connection = builder.connect(loop);
  ...
  for (size_t i = 0; i < 1; ++i) {
    ...
    header._type = method;
    header._path = path;
    header._parameter = parameter;
    header._meta = meta;

    try {
      std::unique_ptr<Request> request(new Request(std::move(header)));
      connection->sendRequest(std::move(request), resCallback, errCallback);
    } catch (std::exception const& ex) {
